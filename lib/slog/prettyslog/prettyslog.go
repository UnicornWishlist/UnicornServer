...
type Colorize interface {
	Level(slog.Level, string) string
...
func (h *Handler) Handle(ctx context.Context, r slog.Record) error {
	...
	if !levelAttr.Equal(slog.Attr{}) {
		...
		level = h.colorize.Level(r.Level, level)
	}
	...
	if !timeAttr.Equal(slog.Attr{}) {
		timestamp = h.colorize.Timestamp(timeAttr.Value.String())
	}
	...
	if !msgAttr.Equal(slog.Attr{}) {
		msg = h.colorize.Message(msgAttr.Value.String())
	}
	...
	if len(attrsAsBytes) > 0 {
		out.WriteString(h.colorize.Attrs(string(attrsAsBytes)))
	}
...
func New(writer io.Writer, handlerOptions *slog.HandlerOptions, colorize Colorize) *Handler {
	if handlerOptions == nil {
		handlerOptions = &slog.HandlerOptions{}
	}

	if colorize == nil {
		colorize = NewDefaultColorize()
	}
